...
# Apporto
variant<-read_excel("C:/Users/zzha736/Downloads/0-Datatable\ export.xlsx")

variantstatic <- variant
variantstatic <- variantstatic %>%
  fill(`Day of Week Ending`, "Lineage Bin") 
...
variantstatic <- variantstatic %>% 
  dplyr::select(-c("Modeltype", "95CI")) %>% 
  rename("weekEnding" = `Day of Week Ending`,
         "regionCode" = `Usa Or Hhsregion`,
          "varianttype"= `Lineage Bin`,
         variant = Variant,
         "proportion" = Share)
...
variantstatic1 <- variantstatic1 %>%
  dplyr::mutate(whoLabel = case_when(
                                     varianttype=="VBM" ~ "Delta",
                                     
                                     varianttype=="VOC" ~ "Omicron",
                                     
                                     TRUE ~ "Other"
  ))


variantstatic1<- variantstatic1[,c(1,7,8,2,6,4,9,5,3)]


variantstatic2 <- variantstatic1 %>% 
  mutate(variant = ifelse(
  whoLabel=="N/A",variant,paste(whoLabel,variant,sep="_"))) %>% 
  dplyr::select(-c("whoLabel", "varianttype"))
...
variant1 <- variant1[,c(1,7,8,2,6,4,9,5,3)]

variant2 <- variantstatic1 %>% 
  mutate(variant = ifelse(
    whoLabel=="N/A",variant,paste(whoLabel,variant,sep="_"))) %>% 
  dplyr::select(-c("whoLabel", "varianttype"))
...
transpose<-stateDeath1 %>% 
  ...
  mutate(FIPS = fips(state),
         FIPS = ifelse(is.na(FIPS),"_nation", FIPS),
         totalDeaths = ifelse(is.na(totalDeaths),-9999, totalDeaths)) %>% 
...
pop2$race[pop2$race=="Hispanic"]<-"Hispanic.or.Latino"
