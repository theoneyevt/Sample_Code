...
filename first temp;
		...
		filename forth temp;
		proc http
		 url="https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-counties-2023.csv"
		 method="GET"
		 out=third;
		run;
		proc import
		  file=forth
		  out=work.countiesnytimes2023 replace
		  dbms=csv;
		run;
    data countiesnytimes;
	set countiesnytimes2020 countiesnytimes2021 countiesnytimes2022 countiesnytimes2023;
	run;
		...
		*Cumulative deaths due to covid by county, state, and nation;
			...
			where county=.;
			var date state cases deaths county;
			run;
